...
async function init() {
  ...
  if (retentionOpt)
    retentionOpt.text = `Retention Policy (${retentionDays} Days)`;
  ...
  data.forEach((item) => {
    ...
    for (let i = 0; i < displayBars; i++) {
      ...
      if (bucketRecords.length === 0) {
        ...
        const avgLatency =
          bucketRecords.reduce((acc, r) => acc + (r.latency_ms || 0), 0) /
    ...
    const uptime =
      totalRecs > 0 ? ((totalSuccess / totalRecs) * 100).toFixed(1) : "0.0";
...
function renderGlobalEventLog(data) {
  ...
  if (recentEvents.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="4" style="text-align:center; color:var(--text-muted); padding:20px;">No status changes recorded in current history.</td></tr>';
    return;
  }
...
function startDashboardUpdates() {
  ...
  eventSource.addEventListener("update", (e) => {
    try {
      ...
      const index = monitorData.findIndex(
        (item) => item.target.id === updatedStatus.target.id,
      );
      if (index !== -1) {
...
function renderConfigForm() {
  ...
  document.getElementById("config-retention").value =
    currentConfig.data_retention_days || 3;
}
...
function addConfigRow(target) {
  ...
  const protoOptions = ["TCP", "ICMP", "DNS", "HTTP", "HTTPS"]
    .map(
      (p) =>
...
function addWebhookRow(webhook) {
  ...
  const defTmpl =
    '{"chat_id":"<your_chat_id>","text":"{{STATUS}} {{TARGET}} {{HOST}} {{TIME}} {{MESSAGE}}"}';
...
async function saveConfig() {
  ...
  const newConfig = {
    ...
    data_retention_days:
      parseInt(document.getElementById("config-retention").value) || 3,
  };
